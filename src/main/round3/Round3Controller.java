...
public class Round3Controller extends MainController implements Initializable, Runnable, TimerInterface {
    @FXML
    ...
    private void handleMouseClick(MouseEvent e) {
        if (e.getSource() == bt_level1Instruction) {
            ...
            writeToClient(Constants.DIS_R3L1_INS, Constants.LEVEL1);
        } else if (e.getSource() == bt_level1Example) {
            hide();
            ap_level1InterfaceController.showExample();
            writeToClient(Constants.DIS_R3L1_EXP, Constants.LEVEL1);
        } else if (e.getSource() == bt_level2Instruction) {
            hide();
            ap_level2InterfaceController.showInstruction();
            writeToClient(Constants.DIS_R3L2_INS, Constants.LEVEL2);
        } else if (e.getSource() == bt_level2Example) {
            hide();
            ap_level2InterfaceController.showExample();
            writeToClient(Constants.DIS_R3L2_EXP, Constants.LEVEL2);
        } else if (e.getSource() == bt_level3Instruction) {
            hide();
            ap_level3InterfaceController.showInstruction();
            writeToClient(Constants.DIS_R3L3_INS, Constants.LEVEL3);
        } else if (e.getSource() == bt_level3Example) {
            hide();
            ap_level3InterfaceController.showExample();
            writeToClient(Constants.DIS_R3L3_EXP, Constants.LEVEL3);
        } else if (e.getSource() == bt_startTimer) {
            writeToClient(Constants.DIS_R3L1_QST, Constants.LEVEL1);
            new Timer(lb_level1Timer, Main.R3L1_DATA.TIME_LIMIT * 60, this, 0);
            writeToClient(Constants.DIS_R3L2_QST, Constants.LEVEL2);
            new Timer(lb_level2Timer, Main.R3L2_DATA.TIME_LIMIT * 60, this, 0);
            writeToClient(Constants.DIS_R3L3_QST, Constants.LEVEL3);
            new Timer(lb_level3Timer, Main.R3L3_DATA.TIME_LIMIT * 60, this, 0);
