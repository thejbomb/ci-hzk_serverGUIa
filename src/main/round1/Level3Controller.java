...
public class Level3Controller extends Round1Controller implements Initializable, Runnable {
    ...
    public void init(LinkedList<UserDataLevel3> level3Users, Round1Controller controller) {
        ...
        if (cb_users != null)
            for (UserDataLevel3 ud : level3Users) {
                if (ud.getUSER_NAME() != null && ud.getUSER_NAME().compareTo((String) cb_users.getValue()) == 0) {
                    ...
                    writeToClient(Constants.S2C_R1L3_SCR, packageData(ud.getRound1Points().getLast()), ud.getThreadId());
                }
            }
    }
    ...
    public void handleClientData(int command, LinkedList<String> data) {
        switch (command) {
            case Constants.C2S_R1L3_ANS:
                ...
                try {
                    for (int i = 0; i < answers.size(); i++) {
                        for (String pl : level3Users.getFirst().getRound1Answers()[i]) {
                            Label label = new Label(pl);
                            answers.get(i).getChildren().addAll(label);
                        }
                    }
