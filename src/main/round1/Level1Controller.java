...
public class Level1Controller extends Round1Controller implements Initializable, Runnable {
    ...
    public void init(LinkedList<UserDataLevel1> level1Users, Round1Controller controller) {
    ...
    public void handleClientData(int command, LinkedList<String> data) {
        switch (command) {
            case Constants.C2S_R1L1_ANS:
                ...
                try {
                    for (int i = 0; i < answers.size(); i++) {
                        for (String pl : level1Users.getFirst().getRound1Answers()[i]) {
                            Label label = new Label(pl);
                            answers.get(i).getChildren().addAll(label);
                        }
                    }
